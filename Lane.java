...
public class Lane {
	...
	public Lane(int LeftBoundary, int RightBoundary, int numOfObstacles, int length) {
		this.length = length;
	...
	public Lane(int length, OrthographicCamera oc, int laneNum, Lane[] lanes, int LeftBoundary, int RightBoundary, int numOfObstacles) {//, Code c) {
		this.length = length;
		this.LeftBoundary = LeftBoundary;
		this.RightBoundary = RightBoundary;
		this.numOfObstacles = numOfObstacles;
	...
	public void spawnObstacle(HashMap<String, Texture> textures, int x, int y, String obstacleType, int width, int heigth) throws Exception {
		String[] possibleDirs = {"East", "West"};
		Random rnd = new Random();
		Obstacle toAdd = null;
		switch(obstacleType) {
			case("Rock"):
				toAdd = new Rock(textures, x, y, width, heigth);
			case("Log"):
				toAdd = new Log(textures, x, y, width, heigth);
			case("Duck"):
				toAdd = new Duck(textures, x, y, width, heigth, possibleDirs[rnd.nextInt(2)], lanes, laneNum);
		}
